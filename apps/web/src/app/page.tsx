...
import type { EgoState, BoundingBox3D } from '@/lib/types';
...
export default function Dashboard() {
  ...
  const [sceneMetadata, setSceneMetadata] = useState<{ objects: BoundingBox3D[], ego: EgoState }[]>([]); 
  ...
  useEffect(() => {
      const fetchData = async () => {
          try {
            ...
            const res = await fetch(`/api/drive/metadata?file=${selectedFile}`);
  ...
  const url = `/api/points/sample?frame=${currentFrame}&file=${selectedFile}`;
